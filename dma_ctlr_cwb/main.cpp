...
int sc_main(int argc, char** argv)
{
    ...
    tb_DMA u_tb("TB*");
   	...
   	u_dma.SA.CBM_addr(SA_CBM_addr);
   	u_dma.SA.CBM_read_req(SA_CBM_read_req);
   	u_dma.SA.CBM_write_req(SA_CBM_write_req);
   	u_dma.SA.CBM_size(SA_CBM_size);
   	u_dma.SA.CBM_write_data(SA_CBM_write_data);
   	u_dma.SA.CBM_error(SA_CBM_error);
	u_dma.SA.CBM_read_data(SA_CBM_read_data);
   	u_dma.SA.CBM_hmaster(SA_CBM_hmaster);
   	u_dma.SA.CBM_busy(SA_CBM_busy);
   	u_dma.SA.CBM_retry(SA_CBM_retry);
   	u_dma.SA.CBM_split(SA_CBM_split);

    u_dma.MA.CBM_read_req(MA_CBM_read_req);    	
    u_dma.MA.CBM_write_req(MA_CBM_write_req);
    u_dma.MA.CBM_burst(MA_CBM_burst);
    u_dma.MA.CBM_addr(MA_CBM_addr);
    u_dma.MA.CBM_length(MA_CBM_length);
    u_dma.MA.CBM_size(MA_CBM_size);
    u_dma.MA.CBM_lock(MA_CBM_lock);
    u_dma.MA.CBM_write_data(MA_CBM_write_data);
    u_dma.MA.CBM_read_data(MA_CBM_read_data);
    u_dma.MA.CBM_command_busy(MA_CBM_command_busy);
    u_dma.MA.CBM_data_ready(MA_CBM_data_ready);
    u_dma.MA.CBM_error(MA_CBM_error);
    u_dma.MA.CBM_count(MA_CBM_count);
